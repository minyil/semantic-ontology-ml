...
EVIDENCE_LABELS = {
    ...
    "en": {value: value for value in EVIDENCE_CLASSES},
}
KIND_LABELS = {
    "zh-Hant-TW": {
        "control_variable": "控制變數",
        "unmeasured_action": "未量測行動",
        "mechanism": "機制",
        "process_state": "製程狀態",
        "measurement": "量測",
        "constraint": "約束",
    ...
    "en": {value: value for value in CONCEPT_KINDS},
}
PREDICATE_LABELS = {
    "zh-Hant-TW": {
        "measures": "量測",
...
UI_TEXT = {
    "zh-Hant-TW": {
        "domain_ontology": "領域知識本體",
        "analysis_goal": "分析目標",
        "validation": "驗證",
        "error": "錯誤",
        "warning": "警告",
        "no_issues": "未發現結構或證據邊界問題。",
        "concepts": "概念",
        "concept": "概念",
        "kind": "類型",
        "lane": "區段",
        ...
        "target": "目標",
        "lag": "時間差",
        "plant_confirmed": "廠區已確認",
        "evidence_sources": "證據來源",
        "unresolved_questions": "未解問題",
        "none_recorded": "未記錄",
        "interpretation_limits": "解讀限制",
        "unspecified": "未指定",
        "no_locator": "無定位資訊",
        "yes": "是",
        "no": "否",
        "mermaid_note": "節點色彩表示概念類型；邊線樣式表示證據。虛線與點線仍屬候選關係。",
        "selected_node": "選取的本體節點",
        "model_review": "模型審查",
        "definition": "定義",
        "details": "細節",
        "evidence_links": "證據連結",
        "fields": "欄位",
        "sources": "來源",
        "no_description": "未提供說明。",
        "diagram_kicker": "領域知識本體",
        "diagram_note": "節點類型組織領域敘事；邊線樣式保留已宣告、已觀測、推論與建議的證據邊界。",
        "evidence_legend": "證據圖例",
        "review_graph": "審查圖",
        "related_to": "相關",
    },
    "en": {
        "domain_ontology": "Domain knowledge ontology",
        "analysis_goal": "Analysis goal",
        "validation": "Validation",
        "error": "ERROR",
        "warning": "WARNING",
        "no_issues": "No structural or evidence-boundary issues detected.",
        "concepts": "Concepts",
        "concept": "Concept",
        "kind": "Kind",
        "lane": "Lane",
        ...
        "yes": "yes",
        "no": "no",
        "mermaid_note": "Node color shows concept kind; edge style shows evidence. Dashed and dotted edges remain candidates.",
        "selected_node": "SELECTED ONTOLOGY NODE",
        ...
        "fields": "Fields",
        "sources": "Sources",
        "no_description": "No description supplied.",
        "diagram_kicker": "DOMAIN KNOWLEDGE ONTOLOGY",
        "diagram_note": "Node kind organizes the domain narrative; edge style preserves declared, observed, inferred, and proposed evidence.",
        "evidence_legend": "Evidence legend",
        "review_graph": "review graph",
...
INTERPRETATION_LIMITS = {
    "zh-Hant-TW": [
        "已觀測的時間順序與共同出現不能證明因果。",
        "一般領域參考資料僅支持推論機制，不代表特定廠區已宣告的事實。",
        "建議的原因、風險與行動仍需領域負責人或來源系統驗證。",
    ],
    "en": [
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="User-facing output language; defaults to Traditional Chinese",
    )
...
def localized_kind(value: str, language: str) -> str:
    label = KIND_LABELS[language].get(value, value)
    return f"{label} (`{value}`)" if language == DEFAULT_LANGUAGE and value else label


def localized_lane(value: str, language: str) -> str:
    label = LANE_LABELS[language].get(value, value)
    return f"{label} (`{value}`)" if language == DEFAULT_LANGUAGE and value else label


def relation_label(relation: dict[str, Any], language: str) -> str:
    label = relation.get("label")
    if isinstance(label, str) and label.strip():
        return label.strip()
    predicate = str(relation.get("predicate", "")).strip()
    return PREDICATE_LABELS[language].get(predicate, predicate or ui(language, "related_to"))
...
def profile(model: dict[str, Any], focus: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "interpretation_limits": INTERPRETATION_LIMITS["en"],
    }
...
def markdown(
    ...
    lines = [f"# {meta.get('name') or ui(language, 'domain_ontology')}", ""]
    if meta.get("description"):
        lines.extend([str(meta["description"]), ""])
    if meta.get("analysis_goal"):
        lines.extend([f"## {ui(language, 'analysis_goal')}", "", str(meta["analysis_goal"]), ""])
    lines.extend(
        [
            f"## {ui(language, 'validation')}",
            "",
            *([f"- {ui(language, 'error')}: {item}" for item in validation["errors"]] or []),
            *([f"- {ui(language, 'warning')}: {item}" for item in validation["warnings"]] or []),
        ]
    )
    if not validation["errors"] and not validation["warnings"]:
        lines.append(f"- {ui(language, 'no_issues')}")
    lines.extend(
        [
            "",
            f"## {ui(language, 'concepts')}",
            "",
            f"| {ui(language, 'concept')} | {ui(language, 'kind')} | {ui(language, 'lane')} | "
            f"{ui(language, 'evidence')} | {ui(language, 'confidence')} | {ui(language, 'source_fields')} |",
            "|---|---|---|---|---:|---|",
        ]
    )
    for concept in model.get("concepts", []):
        ...
        lane = concept_lane(concept)
        lines.append(
            f"| {concept.get('name', concept.get('id'))} | {localized_kind(kind, language)} | "
            f"{localized_lane(lane, language)} | {localized_evidence(evidence_value(concept), language)} | "
            f"{confidence} | {fields} |"
        )
    lines.extend(
        [
            "",
            f"## {ui(language, 'relationships')}",
            "",
            f"| {ui(language, 'source')} | {ui(language, 'relationship')} | {ui(language, 'target')} | "
            f"{ui(language, 'evidence')} | {ui(language, 'confidence')} | {ui(language, 'lag')} | "
            f"{ui(language, 'plant_confirmed')} |",
            "|---|---|---|---|---:|---|---|",
        ]
    )
    names = {item.get("id"): item.get("name", item.get("id")) for item in model.get("concepts", [])}
    for relation in model.get("relationships", []):
        label = relation_label(relation, language)
        predicate = str(relation.get("predicate", ""))
        relationship = f"{label} (`{predicate}`)" if label != predicate and predicate else label
        lines.append(
            f"| {names.get(relation.get('source'), relation.get('source'))} | "
            f"{relationship} | "
            f"{names.get(relation.get('target'), relation.get('target'))} | "
            f"{localized_evidence(evidence_value(relation), language)} | {relation.get('confidence', '')} | "
            f"{relation.get('temporal_lag') or ''} | "
            f"{ui(language, 'yes') if relation.get('plant_confirmed') is True else ui(language, 'no')} |"
        )
    lines.extend(["", f"## {ui(language, 'evidence_sources')}", ""])
    for source in model.get("evidence_sources", []):
        lines.append(
            f"- `{source.get('id')}` — {source.get('kind', ui(language, 'unspecified'))} / "
            f"{source.get('authority', ui(language, 'unspecified'))}: {source.get('description', '')} "
            f"({source.get('locator', ui(language, 'no_locator'))})"
        )
    lines.extend(["", f"## {ui(language, 'unresolved_questions')}", ""])
    questions = model.get("unresolved_questions", [])
    lines.extend(f"- {item}" for item in questions)
    if not questions:
        lines.append(f"- {ui(language, 'none_recorded')}。" if language == DEFAULT_LANGUAGE else f"- {ui(language, 'none_recorded')}.")
    lines.extend(["", f"## {ui(language, 'interpretation_limits')}", ""])
    lines.extend(f"- {item}" for item in INTERPRETATION_LIMITS[language])
    return "\n".join(lines) + "\n"
...
def mermaid(
    ...
    for lane in LANES:
        ...
        if not lane_concepts:
            continue
        lines.append(f'    subgraph {lane}["{mermaid_escape(LANE_LABELS[language][lane])}"]')
        for concept in lane_concepts:
            label = (
                f"{mermaid_escape(concept.get('name', concept['id']))}<br/>"
                f"{mermaid_escape(localized_kind(str(concept.get('kind', '')), language))} · "
                f"{mermaid_escape(localized_evidence(evidence_value(concept), language))}"
            )
    ...
    for index, relation in enumerate(model.get("relationships", [])):
        ...
        lines.append(
            ...
            f'[{mermaid_escape(localized_evidence(evidence, language))}]'
            f'{mermaid_escape(suffix)}"| {ids[relation["target"]]}'
        )
...
def svg_card(
    ...
    evidence = evidence_value(concept)
    evidence_text = localized_evidence(evidence, language)
    parts = [
...
def svg_edge(
    ...
    lag = relation.get("temporal_lag")
    evidence_text = localized_evidence(evidence, language)
    lag_label = ui(language, "lag")
    full_label = f"{label} [{evidence_text}]" + (f"; {lag_label} {lag}" if lag else "")
...
def svg_detail_panel(
    ...
    parts = [
        ...
        svg_text(
            x + 28,
            y + 42,
            ui(language, "selected_node") if focus else ui(language, "model_review"),
            13,
            "#0f766e",
            700,
        ),
    ...
    if focus:
        ...
        parts.append(
            svg_text(
                x + 28,
                cursor + 5,
                f"{localized_kind(str(focus.get('kind', '')), language)} · "
                f"{localized_evidence(evidence_value(focus), language)}",
                14,
                "#0f766e",
                700,
            )
        ...
        cursor += 45
        parts.append(svg_text(x + 28, cursor, ui(language, "definition"), 15, "#0f766e", 700))
        cursor += 30
        for line in wrap(focus.get("description", ui(language, "no_description")), 44)[:6]:
            parts.append(svg_text(x + 28, cursor, line, 14, "#405466"))
            cursor += 23
        details = focus.get("details", []) if isinstance(focus.get("details"), list) else []
        if details:
            cursor += 20
            parts.append(svg_text(x + 28, cursor, ui(language, "details"), 15, "#0f766e", 700))
            cursor += 30
        ...
        cursor += 18
        parts.append(
            svg_text(x + 28, cursor, ui(language, "evidence_links"), 15, "#9b3d55", 700)
        )
        cursor += 30
        for label, values in (
            (ui(language, "fields"), source_fields),
            (ui(language, "sources"), source_refs),
        ):
            value = (
                ", ".join(values)
                if isinstance(values, list) and values
                else ui(language, "none_recorded")
            )
        ...
        domain_model = model.get("domain_model", {})
        for line in wrap(domain_model.get("name", ui(language, "domain_ontology")), 30)[:2]:
            parts.append(svg_text(x + 28, cursor, line, 23, "#243b53", 800))
            cursor += 30
        cursor += 15
        parts.append(svg_text(x + 28, cursor, ui(language, "analysis_goal"), 15, "#0f766e", 700))
        cursor += 30
        for line in wrap(domain_model.get("analysis_goal", ui(language, "no_description")), 44)[:6]:
            parts.append(svg_text(x + 28, cursor, line, 14, "#405466"))
            cursor += 23
        cursor += 25
        parts.append(
            svg_text(x + 28, cursor, ui(language, "unresolved_questions"), 15, "#c56a00", 700)
        )
        cursor += 30
...
def svg(
    ...
    title = report["domain_model"].get("name") or ui(language, "domain_ontology")
    parts = [
    ...
    parts.extend(
        [
            ...
            svg_text(42, 38, ui(language, "diagram_kicker"), 14, "#0f766e", 700),
            svg_text(42, 82, title, 32, "#172b3a", 800),
            svg_text(
                42,
                116,
                ui(language, "diagram_note"),
                15,
                "#617386",
            ),
    ...
    for relation in model.get("relationships", []):
        if relation.get("source") in boxes and relation.get("target") in boxes:
            parts.append(
                svg_edge(
                    boxes[relation["source"]],
                    boxes[relation["target"]],
                    relation,
                    language,
                )
            )
    ...
    parts.extend(
        [
            ...
            svg_text(65, footer_y + 28, ui(language, "evidence_legend"), 15, "#243b53", 700),
        ]
    )
    legend_x = 245
    for evidence in EVIDENCE_CLASSES:
        ...
        parts.append(
            svg_text(
                legend_x + 63,
                footer_y + 44,
                localized_evidence(evidence, language),
                14,
                "#405466",
            )
        )
        legend_x += 270
    parts.append(
        svg_text(1915, footer_y + 44, ui(language, "review_graph"), 13, "#7c8b98", 600, "end")
    )
    parts.append("</svg>")
    return "\n".join(parts) + "\n"
...
def main() -> int:
    ...
    if args.format == "markdown":
        output = markdown(model, report, args.language)
    elif args.format == "mermaid":
        output = mermaid(model, report, args.language)
    elif args.format == "svg":
        output = svg(model, report, args.focus, args.language)
    if args.output:
