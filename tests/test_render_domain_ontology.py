...
class DomainOntologyTests(unittest.TestCase):
    ...
    def test_svg_contains_lanes_focus_details_and_evidence_styles(self) -> None:
        ...
        self.assertIn("投入與控制", output)
        self.assertIn("07:48 multivariate", output)
        self.assertIn("deviation", output)
        self.assertIn("選取的本體節點", output)
        self.assertIn("已觀測", output)
    ...
    def test_mermaid_and_markdown_are_auditable(self) -> None:
        ...
        self.assertIn("candidate contributor [建議]", mermaid)
        self.assertIn("linkStyle", mermaid)
        self.assertIn("## 關係", review)
        self.assertIn("控制變數 (`control_variable`)", review)
        self.assertIn("Which equipment boundary applies", review)
        self.assertIn("已觀測的時間順序", review)

    def test_english_override_localizes_renderer_chrome(self) -> None:
        report = MODULE.profile(MODEL, focus="event")
        mermaid = MODULE.mermaid(MODEL, report, language="en")
        review = MODULE.markdown(MODEL, report, language="en")
        output_svg = MODULE.svg(MODEL, report, focus_id="event", language="en")
        self.assertIn("Inputs &amp; controls", output_svg)
        self.assertIn("SELECTED ONTOLOGY NODE", output_svg)
